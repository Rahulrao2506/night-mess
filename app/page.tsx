...
const EMOJI_MAP: Record<string,string> = {
  ...
  "Chole Bhature":"🫔","Cold Coffee":"☕","Dal Makhani":"🫕","Aloo Paratha":"🫓","Maggi":"🍜",
};
...
export default function NightMess() {
  ...
  const fetchMenu = async () => {
    try {
      ...
      if (data.success && data.items.length > 0) {
        setMenuItems(data.items.map((item:any, i:number) => ({
          id: i+1, name: item.name, price: item.price,
          rating: item.rating||4.5, time: item.prepTime||10,
          category: item.category, popular: item.isHot||false,
          isAvailable: item.isAvailable,
          emoji: EMOJI_MAP[item.name]||'🍽️'
        })));
      }
    } catch(err) { setMenuItems(FALLBACK); }
  };
  ...
  // Auto-remove unavailable cart items
  useEffect(() => {
    const unavailable = menuItems.filter(m=>!m.isAvailable).map(m=>m.name);
    const removed = cart.filter(c=>unavailable.includes(c.name));
    if (removed.length > 0) {
      setCart(prev=>prev.filter(c=>!unavailable.includes(c.name)));
      setRemovedItems(removed.map(c=>c.name));
  ...
  useEffect(() => {
    const tick = () => {
      ...
      if((h>=20&&h<=22)||(h>=7&&h<=9)){setCrowdLevel("High");setWaitTime(25);}
      else if((h>=19&&h<20)||(h>=9&&h<=11)){setCrowdLevel("Medium");setWaitTime(15);}
      else{setCrowdLevel("Low");setWaitTime(8);}
    };
    tick();
      ...
      try{
        ...
        if(data.success&&data.orders){
          ...
          data.orders.forEach((o:any)=>{
            if(o.status==='ready'&&!seenOrdersRef.current.has(o._id)){
              notifs.push(`🎉 Token #${o.tokenNumber} is READY for pickup!`);
  ...
  const placeOrder=async()=>{
    ...
    const unavailable=menuItems.filter(m=>!m.isAvailable).map(m=>m.name);
    const valid=cart.filter(c=>!unavailable.includes(c.name));
    if(valid.length===0){alert('All items unavailable.');setCart([]);return;}
    if(valid.length!==cart.length){setCart(valid);alert('Some unavailable items removed. Try again.');return;}
    try{
      const items=cart.map(c=>({name:c.name,price:c.price,quantity:c.qty,prepTime:menuItems.find(m=>m.id===c.id)?.time||10}));
      const res=await fetch(`${API}/api/orders`,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`},body:JSON.stringify({items,totalAmount:cartTotal})});
      const data=await res.json();
      if(data.success){setOrderToken(`#NM${data.order.tokenNumber}`);setOrderPlaced(true);setCart([]);setCartOpen(false);setTimeout(()=>setOrderPlaced(false),5000);}
    ...
    <div style={{minHeight:"100vh",background:"#0a0a0a",color:"#fff",fontFamily:"'DM Sans', sans-serif"}}>
      ...
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(10,10,10,0.85)",backdropFilter:"blur(20px)",borderBottom:"1px solid #1a1a1a",padding:"14px 20px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{cursor:'pointer'}} onClick={()=>window.location.href='/'}>
          <div style={{fontFamily:"'Syne', sans-serif",fontSize:22,fontWeight:800,background:"linear-gradient(135deg, #ff6b35, #ff9a3c)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>🍽️ Night Mess</div>
          <div style={{fontSize:12,color:"#666",marginTop:1}}>Welcome back, {user?.name||'Guest'} 👋</div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{position:'relative'}}>
            <button onClick={()=>setShowNotifications(!showNotifications)} style={{background:"none",border:"none",color:notifications.length>0?"#ff6b35":"#666",cursor:"pointer",position:'relative'}}>
              <Bell size={20}/>
              {notifications.length>0&&<span style={{position:'absolute',top:-4,right:-4,background:'#ff6b35',borderRadius:'50%',width:16,height:16,fontSize:10,display:'flex',alignItems:'center',justifyContent:'center',color:'white',fontWeight:'bold'}}>{notifications.length}</span>}
            </button>
            {showNotifications&&(
              <div style={{position:'absolute',right:0,top:32,background:'#1a1a1a',border:'1px solid #2a2a2a',borderRadius:12,padding:12,minWidth:260,zIndex:200,boxShadow:'0 8px 32px rgba(0,0,0,0.5)'}}>
                ...
                {notifications.length===0?<div style={{fontSize:12,color:'#666',padding:'8px 0'}}>No notifications yet</div>:notifications.map((n,i)=>(
                  <div key={i} style={{fontSize:12,color:'#fff',padding:'8px 0',borderBottom:i<notifications.length-1?'1px solid #2a2a2a':'none'}}>{n}</div>
                ))}
                {notifications.length>0&&<button onClick={()=>{setNotifications([]);seenOrdersRef.current=new Set();}} style={{fontSize:11,color:'#666',background:'none',border:'none',cursor:'pointer',marginTop:8}}>Clear all</button>}
              </div>
            )}
          </div>
        </div>
      ...
      <main style={{maxWidth:480,margin:"0 auto",padding:"0 16px 100px"}}>
        <AnimatePresence>
          {removedItems.length>0&&(
            <motion.div initial={{y:-60,opacity:0}} animate={{y:0,opacity:1}} exit={{y:-60,opacity:0}}
              ...
              ⚠️ Removed (unavailable): {removedItems.join(', ')}
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.1}}
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:12,color:"#888"}}>
              Crowd: <span style={{color:crowdColor,fontWeight:700}}>{crowdLevel}</span>
        ...
        <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4,marginBottom:20,scrollbarWidth:"none"}}>
          {CATEGORIES.map(cat=>(
            <button key={cat} onClick={()=>setCategory(cat)}
              style={{flexShrink:0,padding:"8px 16px",borderRadius:20,border:"1px solid",fontSize:13,fontWeight:500,cursor:"pointer",background:category===cat?"#ff6b35":"transparent",borderColor:category===cat?"#ff6b35":"#222",color:category===cat?"#fff":"#888"}}>
              {cat}
            </button>
          ))}
        </div>
                ...
                <motion.div key={item.id} initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} transition={{delay:i*0.05}}
                  ...
                  <div style={{padding:"12px 12px 14px"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                      <span style={{display:"flex",alignItems:"center",gap:3,fontSize:11,color:"#f59e0b"}}><Star size={10} fill="#f59e0b"/> {item.rating}</span>
                      <span style={{display:"flex",alignItems:"center",gap:3,fontSize:11,color:"#666"}}><Clock size={10}/> {item.time}m</span>
                    </div>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                      ...
                      {item.isAvailable&&(inCart?(
                        ...
                        <button onClick={()=>addToCart(item)} style={{background:"#ff6b35",border:"none",borderRadius:10,width:30,height:30,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#fff"}}>
                          <Plus size={14}/>
                        </button>
                      ))}
                    </div>
        ...
        {filtered.length===0&&<div style={{textAlign:"center",color:"#444",padding:"60px 0",fontSize:14}}>No items found</div>}
      </main>

      <AnimatePresence>
        {cartCount>0&&(
          <motion.button initial={{y:100,opacity:0}} animate={{y:0,opacity:1}} exit={{y:100,opacity:0}} onClick={()=>setCartOpen(true)}
            ...
            <ShoppingCart size={18}/> {cartCount} item{cartCount>1?"s":""} · ₹{cartTotal} <ChevronRight size={16}/>
          </motion.button>
        )}
      </AnimatePresence>
          ...
          <>
            ...
            <motion.div initial={{y:"100%"}} animate={{y:0}} exit={{y:"100%"}} transition={{type:"spring",damping:25,stiffness:300}}
              ...
              <div style={{borderTop:"1px solid #1a1a1a",marginTop:8,paddingTop:16,marginBottom:20}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:13,color:"#888",marginBottom:8}}><span>Subtotal</span><span>₹{cartTotal}</span></div>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:15,fontWeight:700}}><span>Total</span><span style={{color:"#ff9a3c"}}>₹{cartTotal}</span></div>
              </div>
              <button onClick={placeOrder} style={{width:"100%",background:"linear-gradient(135deg, #ff6b35, #ff9a3c)",border:"none",borderRadius:16,padding:"16px",color:"#fff",fontWeight:700,fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
      ...
      <style>{`* { box-sizing: border-box; margin: 0; padding: 0; } ::-webkit-scrollbar { display: none; } @keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.4; } } input::placeholder { color: #444; }`}</style>
    </div>
  );
}